...
def start(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text('I can analyse your Binance portfolio! Enter your api key.')
    return API_KEY


def unknown_command(update: Update, context: CallbackContext):
    update.message.reply_text(text="Sorry, I didn't understand that command. List of commands: /help")
...
class TelegramBotHandler():
    ...
    def set_binance_api_key(self, update: Update, context: CallbackContext):
        ok_api_key = load_config_json('config/telegram_bot/binance_keys.json')['api_key']
        user = update.message.from_user
        user_api_key = update.message.text
        logger.info(f"api key of {user.first_name}: {user_api_key}")

        if user_api_key == ok_api_key:
            update.message.reply_text('Api key is correct, you are able to build reports. Use /get_report')
            logger.info("api key is ok")
            self._is_token_ok = True
        else:
            update.message.reply_text(f'Api key is not correct: {user_api_key}.\nTry again.')
            return API_KEY
    ...
    def get_report(self, update: Update, context: CallbackContext) -> int:
        ...
        html_fpath = make_report(api_keys['api_key'], api_keys['api_secret'], open_file=False)

        with open(html_fpath, 'rb') as html_file:
            response = self._bot.sendDocument(chat_id=update.effective_chat.id, document=html_file)
    ...
    def run(self):
        ...
        dispatcher.add_handler(CommandHandler('set_binance_api_key', self.set_binance_api_key))
